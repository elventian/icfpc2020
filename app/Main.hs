...
findShId r (BPart "cons_2" [h,t]) = if role==r then (shid, pos, vel, fuel, x5) else findShId r t 
    where role = evalBlock' $BApp (BName "car") $ BApp (BName "car") h
          ...
          fuel = evalBlock' $ BApp (BName "car") $ BApp (BName "car") $ BApp (BName "cdr") $ BApp (BName "cdr") $ BApp (BName "cdr") $ BApp (BName "cdr") $ BApp (BName "car") h
...
loop r url key  = do
    ...
    let (shid, pos, vel, fuel, BNum x5) = findShId role shipsnadcommands
    let (BPart "cons_2" [BNum x, BNum y]) = pos
    let (BPart "cons_2" [BNum vx, BNum vy]) = vel
    let (BNum rv) = role
    let (eid, epos, evel,BNum efuel,  _) = findShId (BNum (1-rv)) shipsnadcommands
    let (BPart "cons_2" [BNum ex, BNum ey]) = epos
    let (BPart "cons_2" [BNum evx, BNum evy]) = evel
    let (ax', ay') = (-127 * signum ex, -127 * signum ey)
    let (ax, ay) = if (signum ax' == -1*signum x && signum ay' == -1*signum y) then (ax', -ay') else (ax,ay)
    ...
    if stage == BNum 2
    then return ()
    else do let BMod mes = evalBlock' $  BApp (BName "mod") $ if (ex-x)*(ex-x) + (ey-y)*(ey-y) < 5*5 && efuel < 200 then BList [BNum 4, BNum key, BList[dCmd]] else BList [BNum 4, BNum key, BList[mCmd]]--if (ex-x)*(ex-x) + (ey-y)*(ey-y) < 50*50 && x5 == 0 then BList [BNum 4, BNum key, BList[mCmd, sCmd]] else BList [BNum 4, BNum key, BList[mCmd]]
            r1 <- send url mes
            loop r1 url key
